...
function M.create()
  local p = popup
    ...
    :action("h", "Log HEAD", function(popup)
      local output =
        git.cli.log.format("fuller").args("--graph", unpack(popup:get_arguments())).for_range("HEAD").call_sync()
    ...
    :action("b", "Log all branches", function(popup)
      local output =
        git.cli.log.format("fuller").args("--graph", unpack(popup:get_arguments())).branches.remotes.call_sync()
      LogViewBuffer.new(parse(output)):open()
    end)
    :action("a", "Log all references", function(popup)
      local output = git.cli.log.format("fuller").args("--graph", unpack(popup:get_arguments())).all.call_sync()
      LogViewBuffer.new(parse(output)):open()
    end)
    :new_action_group()
